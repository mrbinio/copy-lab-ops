...
(function () {
  ...
  function drawH(id, labels, data, colors, xTitle) {
    ...
    Chart.defaults.color = "#8a8b93";
    Chart.defaults.borderColor = "#26262c";
  ...
  function renderCharts(s, rows) {
    ...
    drawH("chart-funnel",
      ["Looked at", "Dropped: too new", "Had 60d history", "We simulated", "Profit 60d AND 90d"],
    ...
    drawH("chart-pnl",
      ...
      "Lab $ if you copied at $5 (not live money)"
    );
    const conc = (s.concentration || []).slice(0, 8);
    drawH("chart-conc",
      conc.map(function (c) { return c.name; }),
      conc.map(function (c) { return c.share; }),
      conc.map(function (c) { return c.share > 0.45 ? "#ff5d5d" : "#7aa8ff"; }),
      "0 = many markets · 1 = one event · red > 0.45"
    );
  }
  ...
  function renderChartReads(s, rows) {
    const f = s.funnel_month || {};
    if ($("read-funnel")) {
      $("read-funnel").textContent = "Right now: " + (f.unique || 0) +
        " names on the month board → " + (f.history_lt_60d || 0) +
        " too new → only " + (f.both_windows || 0) +
        " profitable in both 60d and 90d. / Teraz: prawie cała tablica odpada zanim copy ma sens. Ostatni słupek ma być mały — to nie bug.";
    }
    const wait = rows.filter(function (r) { return r.call === "wait"; });
    const names = wait.slice(0, 3).map(function (r) { return r.name; }).join(", ") || "nobody";
    if ($("read-pnl")) {
      $("read-pnl").textContent = "Yellow / WAIT this pass: " + names +
        ". Grey bars can look rich and still be banned. / Żółte to jedyne, o czym w ogóle rozmawiamy. Wysoki szary słupek to nie pick.";
    }
    const hot = (s.concentration || []).filter(function (c) { return c.share > 0.45; }).slice(0, 3)
      .map(function (c) { return c.name; });
    if ($("read-conc")) {
      $("read-conc").textContent = (hot.length
        ? "Red / one-event: " + hot.join(", ") + "."
        : "No red bars in this slice.") +
        " Copying a red name is the same class of bet as invorser. / Czerwony = jeden event. Tego nie wklejasz.";
    }
  }
  ...
  async function loadHunt() {
    try {
      ...
      $("hunt-plain").textContent = "Hunt :20 and :50 UTC (Mac off). Last run checked " +
        (h.checked || 0) + " new wallets. WAIT = almost. NO = reject. Hunt never turns PolyCop on.";
    } catch (e) {
  ...
  async function renderAll() {
    ...
    if ($("rail-cash")) $("rail-cash").textContent = usd(s.cash_usd);
