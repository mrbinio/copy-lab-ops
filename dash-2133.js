...
(function () {
  ...
  function drawH(id, labels, data, colors, xTitle) {
    ...
    Chart.defaults.borderColor = "#d8dde4";
    charts[id] = new Chart($(id).getContext("2d"), {
      type: "bar",
      data: {
        labels: labels,
        datasets: [{ data: data, backgroundColor: colors || "#1f4b99" }]
      },
      options: {
        indexAxis: "y",
  ...
  function renderCharts(s, rows) {
    ...
    drawH("chart-pnl",
      ...
      top.map(function (r) { return r.call === "wait" ? "#d4a017" : "#c5cad3"; }),
    ...
    drawH("chart-conc",
      ...
      conc.map(function (c) { return c.share > 0.45 ? "#b42318" : "#1f4b99"; }),
